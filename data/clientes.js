/*
 * Catálogo de clientes para las páginas personalizadas.
 *
 * Cada registro se inicia vacío e inactivo. Para habilitar una página,
 * completa sus datos y cambia `activo` a `true`.
 * Este archivo se carga directamente en el navegador: no requiere bundler.
 */
window.CONFIGURACION_DETALLES = {
  // WhatsApp general para recibir pedidos. Incluye país y no usa el signo +.
  whatsappVentas: "51934613286"
};


window.CLIENTES = {
  cliente01: {
    id: "cliente01",
    activo: true,
    nombre: "José",
    mensaje: `Para ti, mi persona favorita.

No sé qué lugar llegaré a ocupar en tu vida, pero me gusta haber coincidido contigo.

No quiero prometerte perfección, solo sinceridad, momentos bonitos y la intención de hacerte sonreír siempre que pueda.

Quizá esto solo sea una canción y unas cuantas palabras, pero detrás hay algo muy real:

alguien pensó en ti mientras preparaba cada detalle. ❤️

Y si algún día me preguntas qué quiero contigo…

simplemente quiero descubrirlo a tu lado.`,
    whatsapp: "",
    musica: "assets/audio/cliente01-t.mp3",
    nombreCancion: "Te prometo",
    imagen: "assets/images/cliente01.jpeg",
    imagenes: ["assets/images/cliente01.jpeg"],
    titulo: "Un detalle para José 💛"
  },
  cliente02: {
    id: "cliente02",
    activo: true,
    nombre: "Angelly",
    mensaje: `Amiga, quiero que sepas lo importante que eres para mí. Gracias por estar presente en los buenos momentos, pero sobre todo por no alejarte cuando las cosas se ponen difíciles.

Nuestra amistad está hecha de risas, conversaciones interminables, consejos sinceros y recuerdos que siempre llevaré conmigo. Tal vez no te lo diga todos los días, pero valoro muchísimo tenerte en mi vida.

Deseo que nunca olvides lo increíble, fuerte y especial que eres. Pase lo que pase y sin importar cuánto cambien nuestras vidas, siempre podrás contar conmigo. Gracias por ser más que una amiga: gracias por convertirte en parte de mi familia. 💛


Con cariño, siempre. 💛`,
    whatsapp: "",
    musica: "assets/audio/cliente02 -x.mp3",
    nombreCancion: "gracias por ser mi amiga",
    imagen: "assets/images/cliente02.jpg",
    imagenes: ["assets/images/cliente02.jpg"],
    titulo: "Un detalle para ANGELLY 💛"
  },
  cliente03: {
    id: "cliente03",
    activo: true,
    nombre: "Mateo",
    mensaje: `Hay personas que llegan sin hacer ruido y terminan dejando una huella enorme.

Tú eres una de esas personas.

Quería regalarte algo diferente: un momento para recordarte lo importante que eres y lo bonito que es coincidir contigo.

Que nunca te falten motivos para sonreír, sueños por cumplir y alguien que celebre cada uno de tus pasos.

Este detalle es para ti, con todo mi cariño. 🌻`,
    whatsapp: "",
    musica: "",
    nombreCancion: "",
    imagen: "",
    imagenes: [],
    titulo: "Para Mateo, con cariño 🌻"
  },
  cliente04: {
    id: "cliente04",
    activo: false,
    nombre: "",
    mensaje: "",
    whatsapp: "",
    musica: "",
    nombreCancion: "",
    imagen: "",
    imagenes: [],
    titulo: ""
  },
  cliente05: {
    id: "cliente05",
    activo: false,
    nombre: "",
    mensaje: "",
    whatsapp: "",
    musica: "",
    nombreCancion: "",
    imagen: "",
    imagenes: [],
    titulo: ""
  },
  cliente06: {
    id: "cliente06",
    activo: false,
    nombre: "",
    mensaje: "",
    whatsapp: "",
    musica: "",
    nombreCancion: "",
    imagen: "",
    imagenes: [],
    titulo: ""
  },
  cliente07: {
    id: "cliente07",
    activo: false,
    nombre: "",
    mensaje: "",
    whatsapp: "",
    musica: "",
    nombreCancion: "",
    imagen: "",
    imagenes: [],
    titulo: ""
  },
  cliente08: {
    id: "cliente08",
    activo: false,
    nombre: "",
    mensaje: "",
    whatsapp: "",
    musica: "",
    nombreCancion: "",
    imagen: "",
    imagenes: [],
    titulo: ""
  },
  cliente09: {
    id: "cliente09",
    activo: false,
    nombre: "",
    mensaje: "",
    whatsapp: "",
    musica: "",
    nombreCancion: "",
    imagen: "",
    imagenes: [],
    titulo: ""
  },
  cliente10: {
    id: "cliente10",
    activo: false,
    nombre: "",
    mensaje: "",
    whatsapp: "",
    musica: "",
    nombreCancion: "",
    imagen: "",
    imagenes: [],
    titulo: ""
  },
  cliente11: {
    id: "cliente11",
    activo: false,
    nombre: "",
    mensaje: "",
    whatsapp: "",
    musica: "",
    nombreCancion: "",
    imagen: "",
    imagenes: [],
    titulo: ""
  },
  cliente12: {
    id: "cliente12",
    activo: false,
    nombre: "",
    mensaje: "",
    whatsapp: "",
    musica: "",
    nombreCancion: "",
    imagen: "",
    imagenes: [],
    titulo: ""
  },
  cliente13: {
    id: "cliente13",
    activo: false,
    nombre: "",
    mensaje: "",
    whatsapp: "",
    musica: "",
    nombreCancion: "",
    imagen: "",
    imagenes: [],
    titulo: ""
  },
  cliente14: {
    id: "cliente14",
    activo: false,
    nombre: "",
    mensaje: "",
    whatsapp: "",
    musica: "",
    nombreCancion: "",
    imagen: "",
    imagenes: [],
    titulo: ""
  },
  cliente15: {
    id: "cliente15",
    activo: false,
    nombre: "",
    mensaje: "",
    whatsapp: "",
    musica: "",
    nombreCancion: "",
    imagen: "",
    imagenes: [],
    titulo: ""
  },
  cliente16: {
    id: "cliente16",
    activo: false,
    nombre: "",
    mensaje: "",
    whatsapp: "",
    musica: "",
    nombreCancion: "",
    imagen: "",
    imagenes: [],
    titulo: ""
  },
  cliente17: {
    id: "cliente17",
    activo: false,
    nombre: "",
    mensaje: "",
    whatsapp: "",
    musica: "",
    nombreCancion: "",
    imagen: "",
    imagenes: [],
    titulo: ""
  },
  cliente18: {
    id: "cliente18",
    activo: false,
    nombre: "",
    mensaje: "",
    whatsapp: "",
    musica: "",
    nombreCancion: "",
    imagen: "",
    imagenes: [],
    titulo: ""
  },
  cliente19: {
    id: "cliente19",
    activo: false,
    nombre: "",
    mensaje: "",
    whatsapp: "",
    musica: "",
    nombreCancion: "",
    imagen: "",
    imagenes: [],
    titulo: ""
  },
  cliente20: {
    id: "cliente20",
    activo: false,
    nombre: "",
    mensaje: "",
    whatsapp: "",
    musica: "",
    nombreCancion: "",
    imagen: "",
    imagenes: [],
    titulo: ""
  }
};
